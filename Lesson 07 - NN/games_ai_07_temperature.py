...
We will try to solve the conversion of Celsius degrees to Fahrenheit.

Of course, it would be simple enough to create a conventional Python function that directly performs this calculation,
but that wouldn't be machine learning (and certainly much less fun).

You should NEVER use this approach for this problem.
However, it is great as an introductory problem - it is simple and understandable.
...
# generate train data
celsius = np.array([-30, -25, -10, 0, 10, 20, 35, 50, 70, 100], dtype=float)
...
celsius_test_value = np.array([celsius_test_value], dtype=float)
print("Predicted value:", model.predict(celsius_test_value))
print("Correct value:", celsius_test_value[0] * 1.8 + 32)

print("These are the neuron variables:", model.layers[0].get_weights())
